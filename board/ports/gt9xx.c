...
#define GTP_IRQ_PIN GET_PIN(D,13)
struct id_touch last_touch={0,0};
...
static void Goodix_TS_Work_Func(void)
{
    ...
    if(input_x!=last_touch.x&&input_y!=last_touch.y){
			last_touch.x = input_x;   
			last_touch.y = input_y;
			rt_kprintf("x = %d, y = %d\r\n",last_touch.x,last_touch.y);
			//status[];
		}
...
//检测到触摸中断时调用，
void GTP_TouchProcess(void *args)
{
  GTP_DEBUG_FUNC();
  Goodix_TS_Work_Func();
...
int Touch_Init(void){
	GTP_Init_Panel();
	rt_pin_mode(GTP_IRQ_PIN,PIN_MODE_INPUT_PULLUP);
	rt_pin_attach_irq(GTP_IRQ_PIN,PIN_IRQ_MODE_RISING, GTP_TouchProcess, RT_NULL);
	I2C_GTP_IRQEnable();
	return 0;
}

INIT_DEVICE_EXPORT(Touch_Init);
